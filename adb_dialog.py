...
class AdbSyncDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.ip_input.setPlaceholderText("Enter device IP for Wi-Fi")
        self.enable_wifi_btn = QPushButton("Enable & Connect Wireless")
        self.enable_wifi_btn.clicked.connect(self.enable_wifi)
        options_layout.addWidget(self.ip_input)
        options_layout.addWidget(self.enable_wifi_btn)
    ...
    def _start_worker(self, command, *args):
        ...
        # Connect signals to their dedicated slots
    ...
    def enable_wifi(self):
        device, ip = self.device_combo.currentText(), self.ip_input.text().strip()
        if not device:
            self.log_output.append("Error: Select a device connected via USB first.")
            return
        if not ip:
            self.log_output.append("Error: Please enter the device's IP address.")
            return
        self._start_worker("enable_wifi", device, ip)
    ...
    def on_worker_finished(self):
        """Generic cleanup slot, called after any command finishes."""
        self.is_running = False
    ...
    def set_controls_enabled(self, enabled):
        ...
        self.enable_wifi_btn.setEnabled(enabled)
